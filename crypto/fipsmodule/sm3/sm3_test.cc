...
void RunSM3(const uint8_t *input, const uint8_t *expected, size_t length) {
  ...
  SM3_CTX ctx;
  EXPECT_EQ(1, SM3_Init(&ctx));
  EXPECT_EQ(1, SM3_Update(&ctx, input, length));
  EXPECT_EQ(1, SM3_Final(digest2, &ctx));
