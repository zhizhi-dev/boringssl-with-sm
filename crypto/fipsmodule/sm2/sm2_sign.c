...
int SM2_compute_z_digest(uint8_t *out, const EVP_MD *digest, const uint8_t *id,
...
static BIGNUM *sm2_compute_msg_hash(const EVP_MD *digest, const EC_KEY *key,
  ...
  const unsigned md_size = EVP_MD_size(digest);
  uint8_t *z = NULL;
  BIGNUM *e = NULL;
...
static ECDSA_SIG *sm2_sig_gen(const EC_KEY *key, const BIGNUM *e) {
   ...
   * A7: Convert the type of data (r,s) to be bit strings according to the
   * details in clause 4.2.2 of GM/T 0003.1-2012. Then the signature of message
   * M is (r,s).
   */
  for (;;) {
    ...
    if (!BN_add(s, dA, BN_value_one()) ||
        !bn_mod_inverse_secret_prime(s, s, s, ctx, &group->order) ||
        !BN_mod_mul(tmp, dA, r, order, ctx) || !BN_sub(tmp, k, tmp) ||
...
int SM2_do_verify(const EC_KEY *key, const EVP_MD *digest, const ECDSA_SIG *sig,
                  const uint8_t *id, const size_t id_len, const uint8_t *msg,
                  size_t msg_len) {
  BIGNUM *e = NULL;
  int ret = 0;
