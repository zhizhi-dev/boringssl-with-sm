...
TEST(SM4Test, ecb) {
  static const uint8_t k[SM4_BLOCK_SIZE] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab,
  ...
  EXPECT_EQ(0, CRYPTO_memcmp(block, expected, SM4_BLOCK_SIZE));

  for (int i = 0; i != 999999; ++i) {
    SM4_encrypt(block, block, &key);
  }
  EXPECT_EQ(0, CRYPTO_memcmp(block, expected_iter, SM4_BLOCK_SIZE));

  for (int i = 0; i != 1000000; ++i) {
    SM4_decrypt(block, block, &key);
  }
  EXPECT_EQ(0, CRYPTO_memcmp(block, input, SM4_BLOCK_SIZE));
}